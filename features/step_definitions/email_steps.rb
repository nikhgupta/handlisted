...
Then /^(?:I|they|"([^"]*?)") should receive (an|no|\d+) emails? with subject "([^"]*?)"$/ do |address, amount, subject|
  emails = unread_emails_for(address).select do |m|
...
Then /^there should be (an|no|\d+) attachments? named "([^"]*?)"$/ do |amount, filename|
  emails = current_email_attachments.select{ |a| a.filename == filename }
  expect(emails.size).to eq(parse_email_count(amount))
end
...
Then /^attachment (\d+) should be of type "([^"]*?)"$/ do |index, content_type|
  content_type = current_email_attachments[(index.to_i - 1)].content_type
  expect(content_type).to include(content_type)
end
