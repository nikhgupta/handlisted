...
RSpec.feature "liking a product" do
  ...
  it "displays errors when like can not be toggled", :js do
    ...
    allow_any_instance_of(User).to receive(:like).with(@product).and_return nil
    toggle_like_for_product(@product)
    expect(@user).not_to be_liking(@product)
    expect(like_status_for(@product)).to be_falsey
    expect(current_path).to eq products_path
    expect(page).to have_selector ".alert", text: "try again"
  end
end
